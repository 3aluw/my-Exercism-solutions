...
def letter_grades(highest):
    ...
    """
    letter_grades_list = []
    loop_step = round((highest - 40)/4)
    for letter_grade in range(41,highest,loop_step):
...
def student_ranking(student_scores, student_names):
    ...
    for index,score in enumerate(student_scores):
        result_string = f"{index+1}. {student_names[index]}: {score}"
        results_array.append(result_string)
